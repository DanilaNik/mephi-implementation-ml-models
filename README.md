...
Сессионный проект по дисциплине «Внедрение моделей машинного обучения»
...
| Сплит трафика           | 50/50 по `user_id`, стабильный (md5 -> bucket)                  |
| Минимальная длительность | 14 дней                                                        |
| Минимальный размер выборки | 1000 предсказаний на группу                                  |
| Ключ группы             | `ab_group` = `control` (v1) либо `treatment` (v2)              |
